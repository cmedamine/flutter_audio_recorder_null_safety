...
class FlutterAudioRecorder {
  static const MethodChannel _channel = MethodChannel('flutter_audio_recorder');
  static const DEFAULT_EXTENSION = '.m4a';
  ...
  Future _init(String? path, AudioFormat? audioFormat, int sampleRate) async {
    ...
    if (path != null) {
      ...
      File file = _fileSystem.file(path);
  ...
  /// a file will be generated in user's file systemggggggggg
  Future start() async {
  ...
  void _responseToRecording(Map<String, Object>? response) {
    ...
    _recording?.audioFormat = _stringToAudioFormat(
      response['audioFormat'] as String,
    );
